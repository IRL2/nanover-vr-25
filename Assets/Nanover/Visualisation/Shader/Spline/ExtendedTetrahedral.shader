Shader "Nanover/Spline/ExtendedTetrahedral"
{
    ...
    /*
     * This shader functions similar to the standard `Tetrahedral` shader but permits the scale of
     * each segment to be set individually. This allows for each residue of a protein to be modified
     * independently.
     */
    SubShader
    {
        ...
        v2f vertexTransformation(appdata v)
        {
            v2f o;
            ...
            float localRadius = lerp(curve.startRadius, curve.endRadius, smoothstep(0, 1, bias));

            v.vertex.xz = 0.5 * _Radius * localRadius * vNorm;
            ...
            v.normal.y = 0;
            v.normal.xz = normalize(0.5 * _Radius * localRadius * vNorm + dd1 * dot(dd1Norm, vNorm) + dd2 * dot(dd2Norm, vNorm));
