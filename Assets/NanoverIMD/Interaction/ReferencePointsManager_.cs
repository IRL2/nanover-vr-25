...
{
    public class ReferencePointsManager_: MonoBehaviour
    {
        [SerializeField]
        ...
        private void Update()
        {
            if (userPointer == null)
            {
                GameObject.FindObjectsByType<ControllerPivot>(FindObjectsSortMode.None)
                    ...
                    .ToList()
                    .ForEach(x => userPointer = x.transform );
                return;
            }
            ...
            if (triggerButton.IsPressed)
            {
                if (!triggerButtonPrevPressed)
                {
                    ...
                    RestartLine();
                    //ProcessFrameData();
                    //addReferencePoint(userPointer.position);
                    addReferencePointFromCursor();
        ...
        private Gradient GetShortGradientFromArray(float[] workSnaps)
        {
            ...
            {
                float segmentSum = 0;
                int segmentStart = (i == 0 ? 1 : i) * segmentSize;
                int segmentEnd = (i == 6) ? workSnaps.Length : segmentStart + segmentSize;

                for (int j = segmentStart; j < segmentEnd; j++)
                {
                    // taks the difference between the current and the previous work value
                    segmentSum += Mathf.Clamp(workSnaps[j] - workSnaps[j-1], 0f, 10000f);
                }
        ...
        {
            label.text = userPointer.transform.ToString();
